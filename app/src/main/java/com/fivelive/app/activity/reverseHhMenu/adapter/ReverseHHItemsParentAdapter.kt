...
class ReverseHHItemsParentAdapter(var context: Context?, var categoryArrayList: List<Category>?) :
    RecyclerView.Adapter<ReverseHHItemsParentAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.parentCategoryTextView.text = category.categoryName
        holder.categoryItemCount.text = "" + category.categoryData.size + " Items"
        holder.parentConstraintLayout.setOnClickListener {
            if (holder.childRecyclerView.visibility == View.VISIBLE) {
                ...
                holder.expendableImageView.setImageDrawable(context!!.resources.getDrawable(R.drawable.up_aa))
            }
        }
        setDataOnList(holder, category.categoryData)
    }

    fun setDataOnList(holder: MyViewHolder, categoryDataList: List<CategoryData>) {
        val adapter = ReverseHHItemsChildAdapter(context, categoryDataList)
